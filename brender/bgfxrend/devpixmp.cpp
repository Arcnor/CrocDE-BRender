...
br_error create_pixelmap(br_device_pixelmap **newpm, br_object *parent, br_token use_type,
...
static struct br_tv_template_entry devicePixelmapTemplateEntries[] = {
		{BRT_WIDTH_I32,               nullptr, F(pm_width),        BRTV_QUERY | BRTV_ALL, BRTV_CONV_I32_U16,},
		{BRT_HEIGHT_I32,              nullptr, F(pm_height),       BRTV_QUERY | BRTV_ALL, BRTV_CONV_I32_U16,},
		{BRT_PIXEL_TYPE_U8,           nullptr, F(pm_type),         BRTV_QUERY | BRTV_ALL, BRTV_CONV_I32_U8,},
		{BRT_OUTPUT_FACILITY_O,       nullptr, F(output_facility), BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,},
		{BRT_FACILITY_O,              nullptr, F(output_facility), BRTV_QUERY,            BRTV_CONV_COPY,},
		{BRT_IDENTIFIER_CSTR,         nullptr, F(pm_identifier),   BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,},
		{BRT_MSAA_SAMPLES_I32,        nullptr, F(msaa_samples),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,}
};
#undef F
...
static struct br_tv_template_entry pixelmapNewTemplateEntries[] = {
		{BRT_MSAA_SAMPLES_I32, nullptr, F(msaa_samples), BRTV_SET, BRTV_CONV_COPY,}
};
#undef F

static void actually_patch_quad(br_device_pixelmap_bgfx_quad *self,
...
const static int s_QuadStride = sizeof(br_device_pixelmap_bgfx_tri);

/* Quad GL_TRIANGES indices. */
static int s_QuadIndices[] = {0, 1, 2, 2, 3, 0};

/* Default quad data. Format is XYZRGBUV. */
static br_device_pixelmap_bgfx_tri s_QuadData[4] = {
		{
				/* Bottom-left */
				-1.0f, -1.0f, 0.5f,
				1.0f, 1.0f, 1.0f,
 ...
 */
br_device_pixelmap *DevicePixelmapBGFXAllocate(br_device *device, br_output_facility *facility, br_token_value *tv)
{
	br_device_pixelmap       *self;
	struct pixelmapNewTokens pt = {0};
	br_int_32                count;
	br_error                 err;

	if(device->templates.pixelmapNewTemplate == nullptr) {
	...
	BrTokenValueSetMany(&pt, &count, nullptr, tv, device->templates.pixelmapNewTemplate);
...
void BR_CMETHOD_DECL(br_device_pixelmap_bgfx, free)(br_object *_self)
{
	auto *self = (br_device_pixelmap *)_self;
...
const char *BR_CMETHOD_DECL(br_device_pixelmap_bgfx, identifier)(br_object *self)
{
	return ((br_device_pixelmap *)self)->pm_identifier;
}

br_token BR_CMETHOD_DECL(br_device_pixelmap_bgfx, type)(br_object *self)
{
	(void)self;
	return BRT_DEVICE_PIXELMAP;
}

br_boolean BR_CMETHOD_DECL(br_device_pixelmap_bgfx, isType)(br_object *self, br_token t)
{
	(void)self;
	return (t == BRT_DEVICE_PIXELMAP) || (t == BRT_OBJECT);
}

br_device *BR_CMETHOD_DECL(br_device_pixelmap_bgfx, device)(br_object *self)
{
	(void)self;
	return ((br_device_pixelmap *)self)->device;
}

br_size_t BR_CMETHOD_DECL(br_device_pixelmap_bgfx, space)(br_object *self)
{
	(void)self;
...
struct br_tv_template *BR_CMETHOD_DECL(br_device_pixelmap_bgfx, templateQuery)(br_object *_self)
{
	auto *self = (br_device_pixelmap *)_self;

	if(self->device->templates.devicePixelmapTemplate == nullptr)
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, resize)(br_device_pixelmap *self, br_int_32 width, br_int_32 height)
{
	/* Front buffer, ¯\_(ツ)_/¯ */
...
static struct br_tv_template_entry pixelmapMatchTemplateEntries[] = {
		{BRT_WIDTH_I32,      nullptr, F(width),      BRTV_SET, BRTV_CONV_COPY,},
		{BRT_HEIGHT_I32,     nullptr, F(height),     BRTV_SET, BRTV_CONV_COPY,},
		{BRT_PIXEL_BITS_I32, nullptr, F(pixel_bits), BRTV_SET, BRTV_CONV_COPY,},
		{BRT_PIXEL_TYPE_U8,  nullptr, F(type),       BRTV_SET, BRTV_CONV_COPY,},
		{BRT_USE_T,          nullptr, F(use_type),   BRTV_SET, BRTV_CONV_COPY,},
};
#undef F

br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, match)(br_device_pixelmap *self,
                                                       br_device_pixelmap **_newpm, br_token_value *tv)
{
	...
	if(self->device->templates.pixelmapMatchTemplate == nullptr) {
	...
	BrTokenValueSetMany(&mt, &count, nullptr, tv, self->device->templates.pixelmapMatchTemplate);
 ...
 */
void _BGFXREND_PreSwapHook(int fbo)
{
	/* nop */
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, doubleBuffer)(br_device_pixelmap *self, br_device_pixelmap *src)
{
...
//
//	/* Call our hook */
//	_BGFXREND_PreSwapHook(src->asBack.glFbo);
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, rectangleStretchCopy)(br_device_pixelmap *self, br_rectangle *d,
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, rectangleCopy)(br_device_pixelmap *self,
                                                               br_point *p, br_device_pixelmap *src, br_rectangle *sr)
{
	...
	return BR_CMETHOD(br_device_pixelmap_bgfx, rectangleStretchCopy)(self, &r, src, sr);
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, rectangleFill)(br_device_pixelmap *self,
...
br_error BR_CMETHOD(br_device_pixelmap_bgfx, rectangleStretchCopyTo)(br_device_pixelmap *self, br_rectangle *dr,
                                                                   br_device_pixelmap *_src, br_rectangle *sr)
{
//	/* Pixelmap->Device, addressable stretch copy. */
//	HVIDEO      hVideo = &self->device->video;
//	auto *src   = (br_pixelmap*)_src;
//	GLuint      tex;
//	br_matrix4  mvp;
//
//	if(self->use_type != BRT_OFFSCREEN)
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, rectangleCopyTo)
		(br_device_pixelmap *self, br_point *p, br_device_pixelmap *src, br_rectangle *sr)
{
	...
	return BR_CMETHOD(br_device_pixelmap_bgfx, rectangleStretchCopyTo)(self, &r, src, sr);
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_bgfx, rectangleCopyFrom)(br_device_pixelmap *self, br_point *p,
...
br_error BR_CMETHOD(br_device_pixelmap_bgfx, text)(br_device_pixelmap *self, br_point *point, br_font *font,
...
static const struct br_device_pixelmap_dispatch devicePixelmapDispatch = {
		...
		.__reserved2                = NULL,
		.__reserved3                = NULL,
		._free                      = BR_CMETHOD_REF(br_device_pixelmap_bgfx, free),
		._identifier                = BR_CMETHOD_REF(br_device_pixelmap_bgfx, identifier),
		._type                      = BR_CMETHOD_REF(br_device_pixelmap_bgfx, type),
		._isType                    = BR_CMETHOD_REF(br_device_pixelmap_bgfx, isType),
		._device                    = BR_CMETHOD_REF(br_device_pixelmap_bgfx, device),
		._space                     = BR_CMETHOD_REF(br_device_pixelmap_bgfx, space),

		._templateQuery             = BR_CMETHOD_REF(br_device_pixelmap_bgfx, templateQuery),
		...
		._resize                    = BR_CMETHOD_REF(br_device_pixelmap_bgfx, resize),
		._match                     = BR_CMETHOD_REF(br_device_pixelmap_bgfx, match),
		...
		._doubleBuffer              = BR_CMETHOD_REF(br_device_pixelmap_bgfx, doubleBuffer),
		...
		._rectangleCopy             = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleCopy),
		._rectangleCopyTo           = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleCopyTo),
		._rectangleCopyFrom         = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleCopyFrom),
		._rectangleStretchCopy      = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleStretchCopy),
		._rectangleStretchCopyTo    = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleStretchCopyTo),
		._rectangleStretchCopyFrom  = BR_CMETHOD_REF(br_device_pixelmap_fail, rectangleStretchCopyFrom),
		._rectangleFill             = BR_CMETHOD_REF(br_device_pixelmap_bgfx, rectangleFill),
		...
		._text                      = BR_CMETHOD_REF(br_device_pixelmap_bgfx, text),
...
//		return err;
	...
	pm = static_cast<br_device_pixelmap *>(BrResAllocate(device, sizeof(br_device_pixelmap), BR_MEMORY_OBJECT));
