...
static void BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, free)(br_object *_self)
{
...
static const char *BR_CMETHOD(br_geometry_v1_buckets_bgfx, identifier)(br_object *self)
{
	return ((br_geometry_v1_buckets *)self)->identifier;
}


static br_token BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, type)(br_object *self)
{
	return BRT_GEOMETRY_V1_BUCKETS;
}

static br_boolean BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, isType)(br_object *self, br_token t)
{
	return (t == BRT_GEOMETRY_PRIMITIVES) || (t == BRT_GEOMETRY) || (t == BRT_OBJECT);
}

static br_device *BR_CMETHOD(br_geometry_v1_buckets_bgfx, device)(br_object *self)
{
	return ((br_geometry_v1_buckets *)self)->device;
}

static br_size_t BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, space)(br_object *self)
{
	return sizeof(br_geometry_v1_buckets);
}

static struct br_tv_template *BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, templateQuery)(br_object *_self)
{
	br_geometry_v1_buckets *self = (br_geometry_v1_buckets *)_self;
...
br_error BR_CMETHOD_DECL(br_geometry_v1_buckets_bgfx, render)(br_geometry_v1_buckets *self, br_renderer *renderer,
...
static const struct br_geometry_v1_buckets_dispatch geometryV1BucketsDispatch = {
		...
		.__reserved2        = NULL,
		.__reserved3        = NULL,
		._free              = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, free),
		._identifier        = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, identifier),
		._type              = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, type),
		._isType            = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, isType),
		._device            = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, device),
		._space             = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, space),

		._templateQuery     = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, templateQuery),
		...
		._render            = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, render),
		._renderOnScreen    = BR_CMETHOD_REF(br_geometry_v1_buckets_bgfx, render),
};
 ...
 */
br_geometry_v1_buckets *GeometryV1BucketsBGFXAllocate(br_renderer_facility *type, const char *id)
{
	br_geometry_v1_buckets *self;

	self = static_cast<br_geometry_v1_buckets *>(BrResAllocate(type, sizeof(*self), BR_MEMORY_OBJECT));
