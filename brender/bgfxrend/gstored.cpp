...
static const struct br_tv_template_entry templateEntries[] = {
		...
		{BRT_GEOMETRY_V1_MODEL, nullptr, F(gv1model),   BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,},
		{BRT_SHARED_B,          nullptr, F(shared),     BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,},
};
#undef F
...
//	glVertexAttribPointer(hVideo->brenderProgram.attributes.aPosition, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
//
//	{ /* gl_vertex_f */
...
static void BR_CMETHOD(br_geometry_stored_bgfx, free)(br_object *_self)
{
...
static const char *BR_CMETHOD(br_geometry_stored_bgfx, identifier)(br_object *self)
{
	return ((br_geometry_stored *)self)->identifier;
}

static br_device *BR_CMETHOD(br_geometry_stored_bgfx, device)(br_object *self)
{
	return ((br_geometry_stored *)self)->device;
}

static br_token BR_CMETHOD(br_geometry_stored_bgfx, type)(br_object *self)
{
	return BRT_GEOMETRY_STORED;
}

static br_boolean BR_CMETHOD(br_geometry_stored_bgfx, isType)(br_object *self, br_token t)
{
	return (t == BRT_GEOMETRY_STORED) || (t == BRT_GEOMETRY) || (t == BRT_OBJECT);
}

static br_size_t BR_CMETHOD(br_geometry_stored_bgfx, space)(br_object *self)
{
	return sizeof(br_geometry_stored);
}


static struct br_tv_template *BR_CMETHOD(br_geometry_stored_bgfx, templateQuery)(br_object *_self)
{
	br_geometry_stored *self = (br_geometry_stored *)_self;

	if(self->device->templates.geometryStoredTemplate == nullptr) {
...
static br_primitive *heapPrimitiveAdd(br_primitive_heap *heap, br_token type)
{
	...
	ASSERT_MESSAGE("Out of heap space", p != nullptr);
...
//
//	return hidden->order_table != nullptr && hidden->heap != nullptr;
//}
...
//			 */
//			if(state->hidden.insert_fn != nullptr) {
...
//}
//
static br_error BR_CMETHOD(br_geometry_stored_bgfx, render)(br_geometry_stored *self, br_renderer *renderer)
{
//	return V1Model_RenderStored(self, renderer, BR_FALSE);
	return BRE_OK;  // FIXME: REMOVE!
}

static br_error BR_CMETHOD(br_geometry_stored_bgfx, renderOnScreen)(br_geometry_stored *self, br_renderer *renderer)
{
...
static const struct br_geometry_stored_dispatch geometryStoredDispatch = {
		...
		.__reserved2        = nullptr,
		.__reserved3        = nullptr,
		._free              = BR_CMETHOD(br_geometry_stored_bgfx, free),
		._identifier        = BR_CMETHOD(br_geometry_stored_bgfx, identifier),
		._type              = BR_CMETHOD(br_geometry_stored_bgfx, type),
		._isType            = BR_CMETHOD(br_geometry_stored_bgfx, isType),
		._device            = BR_CMETHOD(br_geometry_stored_bgfx, device),
		._space             = BR_CMETHOD(br_geometry_stored_bgfx, space),

		._templateQuery     = BR_CMETHOD(br_geometry_stored_bgfx, templateQuery),
		...
		._render            = BR_CMETHOD(br_geometry_stored_bgfx, render),
		._renderOnScreen    = BR_CMETHOD(br_geometry_stored_bgfx, renderOnScreen),
};


br_geometry_stored *GeometryStoredBGFXAllocate(br_geometry_v1_model *gv1model, const char *id, struct v11model *model)
{
	size_t             total_vertices, total_faces;
	br_geometry_stored *self;

	self = static_cast<br_geometry_stored *>(BrResAllocate(gv1model->renderer_facility->object_list, sizeof(*self), BR_MEMORY_OBJECT));
	...
	self->groups = static_cast<gl_groupinfo *>(BrResAllocate(gv1model, sizeof(gl_groupinfo) * model->ngroups, BR_MEMORY_OBJECT_DATA));
